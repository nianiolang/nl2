...
def nlasm::order_t() {
	return ptd::var({
			...
			bin_op => @nlasm::bin_op,
			ov_is => ptd::rec({dest => @nlasm::reg_t, src => @nlasm::reg_t, label => ptd::string(), label_no => ptd::int()}),
			ov_as => ptd::rec({dest => @nlasm::reg_t, src => @nlasm::reg_t, label => ptd::string(), label_no => ptd::int()}),
			return => @nlasm::return,
			die => @nlasm::reg_t,
